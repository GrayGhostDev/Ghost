...
class AuthManager(LoggerMixin):
    ...
    def create_access_token(
        ...
        if expires_delta:
            ...
            expire = datetime.now(timezone.utc) + timedelta(hours=self.expiry_hours)
    ...
    def create_reset_token(self, email: str, user_id: str) -> str:
        """Create a password reset JWT with 1-hour TTL."""
        ...
        token = jwt.encode(payload, self.secret_key, algorithm=self.algorithm)
        self.logger.info(f"Password reset token created for {email}")
        return token

    def verify_reset_token(self, token: str) -> Optional[Dict[str, str]]:
        """Verify a password reset token.

        Returns:
            {'user_id': ..., 'email': ...} or None if invalid/expired.
        """
        try:
            payload = jwt.decode(token, self.secret_key, algorithms=[self.algorithm])
            if payload.get("type") != "password_reset":
                self.logger.warning("Token is not a password reset token")
                return None
            return {"user_id": payload["sub"], "email": payload["email"]}
        except jwt.ExpiredSignatureError:
            self.logger.warning("Reset token has expired")
            return None
        except jwt.InvalidTokenError as e:
            self.logger.warning(f"Invalid reset token: {e}")
            return None
...
class RoleBasedAccessControl:
    ...
    @classmethod
    def has_permission(
        cls, user_roles: List[UserRole], required_role: UserRole
    ) -> bool:
